...
let handler = async (m, { conn, usedPrefix }) => {
    ...
    let str;
    if (m.mentionedJid.length > 0) {
        str = `\`${name2}\` Violó fuertemente a \`${name || who}\`. 🥵`.trim(); // Usamos nombre agendado o número si no está agendado
    } else if (m.quoted) {
        str = `\`${name2}\` Violó fuertemente a \`${name || who}\`. 🥵`; // Mensaje cuando se cita a otro usuario
    } else {
