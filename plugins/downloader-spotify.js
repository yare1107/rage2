...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  if (!text) throw `Uso incorrecto. Ejemplo: ${usedPrefix + command} Bellyache`;
  ...
  try {
    ...
    const info = `✨ *Título:* ${res.name}
🗣️ *Artista:* ${res.artista.join(', ')}
🌐 *Enlace:* ${shortURL}
🎶 *Álbum:* ${res.album}`;
    ...
    m.react('✅️'); // Reacción al finalizar

  } catch (error) {
    m.react('❌'); // Reacción en caso de error
  }
}
